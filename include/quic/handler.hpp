...
How to add code blocks that do NOT run for things that do not satisfy the constexpr:
    ...
    int udp_connect(Address& local, Address& remote, T cert, open_callback on_open, close_callback
on_close)
    {
...
Single constexpr function that can return either a std::array<std::string, N> or a std::string based
on whether N > 1 or == 1:
https://github.com/jagerman/libsession-util-nodejs/blob/node-addon-api/src/utilities.hpp#L34

constexpr function that wraps a callback and does different things based on what the callback
returns:
...
How to pass a number of cert arguments and fwd them along:
    ...
    int udp_connect(Address& local, Address& remote, open_callback on_open, close_callback on_close,
CertArgs&&... cert_args)
    {
        // ...
...
Sample usage flow
    - establish a QUIC connection to remote:port
        - this works with ngtcp2 calls (which in turn will fire into TLSContext callbacks) to do
TLS/QUIC handshaking
...
Sample API Usage 1:
    ...
    auto client1 = net.connect("127.0.0.1:4920"_remote, &tls3, &remote_cert2); // binds to anyaddr
udp handle auto client2 = server1.connect("127.0.0.1:4433"_remote, &remote_cert1); // binds to
127.0.0.1:5440, uses tls1 auto client3 = net.connect("127.0.0.1:7777"_remote,
"127.0.0.1:7778"_local, &local_cert3, &remote_cert3); auto client4 =
client1.connect("127.0.0.1:2222"_remote, &remote_cert1); // binds to client1's addr, uses client1's
tls
...
Client: [server_cert, server_CA, system_CA, server_cert_callback] x [client_key/crt, no client
key/cert]
...
Can create tags around arguments, using templates to extract them to specify arguments however you
want
    ...
    net.connect(local_tls{1,2,3}, remote_addr{"1.2.3.4:5678"}, remote_tls{5,6,7},
local_addr{"10.0.0.1:4444"});
...
None of these specify a local bind address, so on the first call (when constructing c1) we would
create an "any" UDPHandle

    auto c3 = net.client_connect(remote_addr{"127.0.0.3:5678"}, local_addr{"127.0.0.1:1111"});

This specifies an explicit local addr that has not been bound yet, so we create a new UDPHandle for
it.

So somewhere in net we would have an unordered_map<local_addr, shared_ptr<UDPHandle>>, and when we
need the UDPHandle we first check if we already have one for that local_addr (or for the "any" addr)
and, if so, reuse it, otherwise construct a new one.



Just looking ahead a little bit: we're very likely going to want to support other types of buffers,
too; we could make that generic by taking a const char* buf, size_t length, buf_free where buf_free
is some callback we can call to free the associated buffer when done with it.  So then, for
instance, if you had a std::vector<std::byte> on hand you could pass that using: open_stream
...
A better alternative to that lambda would be a std::any; so then there's nothing to call,
libquiciness just destroys the any when it's done with the buffer. With bstring_view
...
i.e. with something like:
    ...
    void send(bstring_view data, std::any keep_alive);
    template <typename Char, std::enable_if_t<sizeof(Char) == 1 && !std::is_same_v<Char, std::byte>,
int> = 0> void send(std::basic_string_view<Char> data, std::any keep_alive) { return
...
So try_send would respect the buffer and refuse if it would get too big; send would be
limit-ignoring. template <typename... Args> bool try_send(Args&&... data) { if (bytes_in_flight >=
max_bytes_in_flight) return false; send(std::forward<Args>(args)...); return true;
    }


- implement something similar to blocked/unblocked s.t. we an signal a stream's local buffer is
full. this is a parameter we can pass in stream creation, like "max_bytes_in_flight" or something
