...
#define CHECK(x) \
    if (x < 0) \
        {log::trace(log_cat, "Function call to %s failed", __func__);}
...
namespace oxen::quic
{
    ...
    extern "C"
    {
        ...
        int
        client_cb_wrapper(gnutls_session_t session, unsigned int htype, unsigned int when, unsigned int incoming, const gnutls_datum_t *msg)
        {
            log::trace(log_cat, "%s called", __PRETTY_FUNCTION__);
    ...
    GNUTLSContext::client_init(GNUTLSCert& cert)
    {
        ...
        if (auto rv = gnutls_certificate_allocate_credentials(&cred); rv < 0)
            log::warning(log_cat, "Client gnutls_certificate_allocate_credentials failed: %s", gnutls_strerror(rv));

        config_client_certs(cert);

        if (auto rv = gnutls_init(&session, GNUTLS_CLIENT); rv < 0)
            log::warning(log_cat, "Client gnutls_init failed: %s", gnutls_strerror(rv));

        if (auto rv = gnutls_set_default_priority(session); rv < 0)
            log::warning(log_cat, "gnutls_set_default_priority failed: %s", gnutls_strerror(rv));
        ...
        if (auto rv = ngtcp2_crypto_gnutls_configure_client_session(session); rv < 0)
            log::warning(log_cat, "ngtcp2_crypto_gnutls_configure_client_session failed: %s", ngtcp2_strerror(rv));
        gnutls_session_set_ptr(session, &conn_ref);

        if (auto rv = gnutls_credentials_set(session, GNUTLS_CRD_CERTIFICATE, cred); rv < 0)
            log::warning(log_cat, "gnutls_credentials_set failed: %s", gnutls_strerror(rv));

        if (auto rv = gnutls_alpn_set_protocols(session, &alpn, 1, GNUTLS_ALPN_MANDATORY); rv < 0)
            log::warning(log_cat, "gnutls_alpn_set_protocols failed: %s", gnutls_strerror(rv));
        ...
        if (auto rv = gnutls_server_name_set(session, GNUTLS_NAME_DNS, "localhost", strlen("localhost")); rv < 0)
            log::warning(log_cat, "gnutls_server_name_set failed: %s", gnutls_strerror(rv));
    ...
    GNUTLSContext::server_init(GNUTLSCert& cert)
    {
        ...
        if (auto rv = gnutls_certificate_allocate_credentials(&cred); rv < 0)
            log::warning(log_cat, "Server gnutls_certificate_allocate_credentials failed: %s", gnutls_strerror(rv));

        config_server_certs(cert);

        if (auto rv = gnutls_init(&session, GNUTLS_SERVER); rv < 0)
            log::warning(log_cat, "Server gnutls_init failed: %s", gnutls_strerror(rv));
        ...
        if (auto rv = ngtcp2_crypto_gnutls_configure_server_session(session); rv < 0)
            log::warning(log_cat, "ngtcp2_crypto_gnutls_configure_server_session failed: %s", ngtcp2_strerror(rv));
    ...
    GNUTLSContext::config_server_certs(GNUTLSCert& cert)
    {
        if (auto rv = gnutls_certificate_set_x509_system_trust(cred); rv < 0)
            log::warning(log_cat, "Set x509 system trust failed with code %s", gnutls_strerror(rv));
        ...
        if (!cert.remotecafile.empty())
        {
            if (auto rv = gnutls_certificate_set_x509_trust_file(cred, cert.remotecafile.c_str(), cert.remoteca_type); rv < 0)
                log::warning(log_cat, "Set x509 trust file failed with code %s", gnutls_strerror(rv));
        }

        if (auto rv = gnutls_certificate_set_x509_key_file(cred, cert.certfile.c_str(), cert.keyfile.c_str(), cert.key_type); rv < 0)
            log::warning(log_cat, "Set x509 key file failed with code %s", gnutls_strerror(rv));
